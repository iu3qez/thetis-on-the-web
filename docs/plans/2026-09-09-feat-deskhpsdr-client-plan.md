...
- ~~La dissolvenza per buffer di `playFloat32Stereo`~~ **Corretta il 2026-09-13, per decisione dell'operatore, fuori dalle unita'.** Attenuava un quarto di ogni buffer ai due bordi, cioe' una modulazione d'ampiezza a 93,75 Hz, la cadenza dei buffer, con bande laterali attorno a -17 dBc su una portante CW: e' la causa piu' probabile di `n9bc/thetis-on-the-web#12`. Ora i campioni passano intatti e le rampe compaiono solo su una discontinuita' vera di riproduzione, da entrambi i lati, con un contatore diagnostico delle discontinuita' raccordate. Non e' una riscrittura dello scheduler, quindi non riapre CLI-08 ne' DEC-01: cambia soltanto dove si applicano le rampe. Verificata in un `OfflineAudioContext` con lo scheduler vero: su buffer contigui le bande laterali scendono a -186 dBc, e sui buchi non resta nessun gradino.
- Lo scheduler audio. La issue `n9bc/thetis-on-the-web#9` descrive un ritardo che peggiora progressivamente e si azzera solo spegnendo e riaccendendo l'audio RX: e' lo scheduler naive gia' noto. Il fork lo eredita e questo piano non lo tocca, perche' riscriverlo e' CLI-08, fuori scopo con DEC-01 = A.

### Acceptance Examples

1. **Audio senza campioni spuri.** Connesso a deskHPSDR, con l'audio TCI attivo, il contatore dei frame audio sale, quello dei frame di tipo sconosciuto resta a zero, e nessun campione di header entra nel flusso. **Corretto il 2026-09-12**: questo esempio chiedeva dieci minuti senza il clic periodico, che nessuna unita' di questo piano puo' ottenere, perche' la modulazione udibile a 93,75 Hz la produceva la dissolvenza per buffer e non l'offset del payload. Dopo la correzione della dissolvenza del 2026-09-13 l'assenza del ronzio e' di nuovo un'attesa legittima, ma appartiene a quella correzione e non a C1.
...
**Stato: implementata il 2026-09-13, verifica al banco ancora da fare.** Il campo sta nella modale Impostazioni e non nella barra di connessione, su richiesta dell'operatore, e offre esattamente i quattro rate che il server accetta, 48, 96, 192 e 384 kHz, con 48 kHz di default come prescritto qui sotto. Lo schema delle impostazioni passa alla versione 3. Il client confronta il rate servito con quello chiesto e registra lo scarto una volta, perche' il server coercia la richiesta senza errore quando un altro client riceve IQ. Verificato senza radio in browser: default, migrazione dello schema, rate non valido, salvataggio dalla modale, comando inviato che segue il campo, scarto registrato una sola volta. I riferimenti di riga qui sotto sono superati dalle modifiche di C1 e C2.
...
6. Ronzio: ascoltando la stessa portante CW, il ronzio a 93,75 Hz non c'e' in TOTW e non c'e' nell'uscita audio di deskHPSDR. Se c'e' in entrambi, la causa non era la dissolvenza. Su un collegamento sano il contatore delle discontinuita' raccordate resta a zero.
