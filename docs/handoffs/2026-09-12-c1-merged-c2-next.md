...
**Aggiornamento del 2026-09-13, piu' tardo del resto di questo documento.** C2 e' implementata, e la dissolvenza per buffer, il vero ronzio, e' stata corretta per decisione dell'operatore: i due commit sono sul branch `c2/iq-samplerate-config`, in PR verso `plan/deskhpsdr-client`. La tabella di stato, la correzione datata sul clic e i passi successivi qui sotto descrivono il momento precedente. Lo stato corrente sta in testa alle sezioni C2 e rinviati del piano. Sempre dal 2026-09-13 `main` non e' piu' tenuto allineato all'originale, che e' abbandonato: e' il tronco, contiene `STRATEGY.md`, e il lavoro di `plan/deskhpsdr-client` confluisce li' via PR. La frase sullo stato dei branch qui sotto descrive il modello precedente. Nota sui numeri: sul fork la #12 e' la PR di C1, mentre `n9bc/thetis-on-the-web#12` e `n9bc/thetis-on-the-web#9` sono issue a monte.
