// ── IQ SAMPLE RATE ──
// The rates the deskHPSDR TCI server accepts; it ignores any other value.
// Every iq_samplerate sent reconfigures the radio unless another client is
// already receiving IQ, in which case the server keeps the rate it streams.
const IQ_SAMPLE_RATES = [48000, 96000, 192000, 384000];
const IQ_DEFAULT_SR = 48000;

function iqRequestedRate() {
  const sr = parseInt(getSettings().iqSampleRate);
  return IQ_SAMPLE_RATES.includes(sr) ? sr : IQ_DEFAULT_SR;
}
...
const S = {
  ...
  // IQ panadapter
  iqOn: false, iqSR: IQ_DEFAULT_SR, iqCentre: 14225000, iqRequestedSR: 0,
};
    ...
    S.ws.onopen    = () => {
      ...
      setTimeout(() => {
        if (S.ws && S.ws.readyState === 1) {
          const sr = iqRequestedRate();
          const cmd = 'iq_samplerate:' + sr + ';';
          S.iqRequestedSR = sr;
          S.ws.send(cmd); netTx(cmd.length);
          S.ws.send('iq_start:0;'); netTx(12);
          S.iqOn = true;
          log('sys', 'IQ panadapter stream started, requested ' + (sr / 1000) + ' kHz');
        }
      }, 500);
...
function parseTCI(msg) {
  ...
  switch (cmd) {
    ...
    case 'iq_samplerate':
      ...
      // The server owns the stream rate: while another client receives IQ it
      // answers a request with the rate already streaming, without an error.
      if (S.iqRequestedSR && S.iqSR !== S.iqRequestedSR) {
        log('sys', 'IQ sample rate: requested ' + (S.iqRequestedSR / 1000) + ' kHz, server serves ' + (S.iqSR / 1000) + ' kHz');
      }
      S.iqRequestedSR = 0;
      break;
    case 'modulation':
      if (args.length >= 2 && args[0] === '0') {
        ...
        updMode();
        // The server does not broadcast the filter it restores for the new mode
        // (`iu3qez/deskhpsdr` #19): ask for it now that the change is applied.
        send('rx_filter_band:0;');
...
// deskHPSDR reports CWL and CWU both as CW (`iu3qez/deskhpsdr` #17): keep the sideband
// the client knows, otherwise CWU, which is what the server applies for a plain "cw".
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (audioDiag.binaryFrames === 0) {
        ...
        log('err', 'Types are counted and dropped by design — check the server stream type.');
...
// Every TCI stream frame carries the standard 64-byte header, and the stream
// type at offset 24 is what decides the route — not the frame size and not the
// sample rate. A frame too short to hold that header can only come from a
// legacy Thetis build with the 8-byte layout, so it reaches the audio handler
// with that offset instead. A type we do not know is counted and dropped:
// there is no fall-through branch that plays unrecognised bytes.
const TCI_HEADER_BYTES    = 64;
const LEGACY_HEADER_BYTES = 8;
const STREAM_IQ           = 0;
const STREAM_RX_AUDIO     = 1;
const STREAM_TX_CHRONO    = 3;
const STREAM_SPECTRUM     = 4;

function dispatchBinaryFrame(buf) {
  audioDiag.binaryFrames++;
  audioDiag.lastSize = buf.byteLength;

  if (buf.byteLength < TCI_HEADER_BYTES) {
    rxAudioFrame(buf, LEGACY_HEADER_BYTES);
    return;
  }

  const dv = new DataView(buf);
  const streamType = dv.getUint32(24, true);
  audioDiag.lastType = streamType;

  switch (streamType) {
    case STREAM_IQ:        handleIQFrame(buf); return;
    case STREAM_RX_AUDIO:  rxAudioFrame(buf, TCI_HEADER_BYTES); return;
    case STREAM_TX_CHRONO: handleTxChrono(dv); return;
    case STREAM_SPECTRUM:
      // Bin spectrum stream. C3 parses it; until then it is counted and
      // dropped, so a server stream left enabled cannot turn into noise.
      audioDiag.spectrumFrames++;
      if (audioDiag.spectrumFrames === 1) log('sys', 'Bin spectrum frames arriving — handler lands with C3');
      return;
  }

  if (!audioDiag.unknownTypes[streamType]) {
    audioDiag.unknownTypes[streamType] = 0;
    log('sys', 'Unknown binary stream type='+streamType+' size='+buf.byteLength+'b — discarded, not played');
  }
  audioDiag.unknownTypes[streamType]++;
  audioDiag.discardedFrames++;
}

// ── RX audio handler ──
// The payload offset comes from the caller: 64 for a standard TCI frame, 8 for
// the legacy Thetis layout. Reading a 64-byte frame from offset 8 is what used
// to push 56 bytes of header into the audio stream.
function rxAudioFrame(buf, offset) {
  if (!S.audioCtx || !S.rxOn) return;
  if (S.audioCtx.state === 'suspended') S.audioCtx.resume();

  if (buf.byteLength <= offset) { audioDiag.shortFrames++; return; }

  const dv = new DataView(buf);
  if (offset === TCI_HEADER_BYTES) {
    // Standard header: payload length in samples at offset 20.
    audioDiag.lastSampleCount = dv.getUint32(20, true);
  } else {
    // Legacy header: uint16 type, uint16 receiver, then a uint32 sample rate.
    const legacyType = dv.getUint16(0, true);
    audioDiag.lastType = legacyType;
    audioDiag.lastSampleCount = dv.getUint32(4, true);
    if (legacyType !== 0 && legacyType !== 1) {
      if (!audioDiag.unknownTypes[legacyType]) {
        audioDiag.unknownTypes[legacyType] = 0;
        log('sys', 'Unknown legacy frame type='+legacyType+' size='+buf.byteLength+'b — discarded, not played');
      }
      audioDiag.unknownTypes[legacyType]++;
      audioDiag.discardedFrames++;
      return;
    }
  }
  ...
  if (audioDiag.audioFrames === 1) {
    log('sys', 'First audio frame: payload at offset '+offset+' size='+buf.byteLength+'b samples='+audioDiag.lastSampleCount);
    let hex = '';
    for (let b = 0; b < Math.min(16, buf.byteLength); b++) hex += dv.getUint8(b).toString(16).padStart(2,'0')+' ';
    log('sys', 'First 16 bytes (hex): ' + hex);
  }

  // Derive the float count from the payload size, never from a header field.
  const floatCount = Math.floor((buf.byteLength - offset) / 4);
  if (floatCount < 2) { audioDiag.shortFrames++; return; }
  playFloat32Stereo(buf, offset, floatCount);
}
...
function startIQ() {
  if (S.iqOn) return;
  S.iqRequestedSR = iqRequestedRate();
  send('iq_samplerate:' + S.iqRequestedSR + ';');
  send('iq_start:0;');
  S.iqOn = true;
}
function stopIQ() {
...
function loadSettingsUI() {
  ...
  v('cfgIqSampleRate', cfg.iqSampleRate || IQ_DEFAULT_SR);
...
function saveSettings() {
  ...
  const cfg = {
    ...
    iqSampleRate:       parseInt(gv('cfgIqSampleRate')) || IQ_DEFAULT_SR,
...
function migrateSettings(cfg) {
  ...
  if (cfg.schemaVersion < 3) {
    if (cfg.iqSampleRate == null) cfg.iqSampleRate = IQ_DEFAULT_SR;
    cfg.schemaVersion = 3;
  }
  return cfg;
}
...
window.addEventListener('load', () => {
  ...
  log('sys','Enter ws://[pc-ip]:40001 above for deskHPSDR, or port 50001 for Thetis, then click CONNECT');
});
